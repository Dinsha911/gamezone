...
function pause(){if(state==="play"){state="paused";turbo=false;$("pauseScreen").classList.remove("hidden")}}function resume(){if(state==="paused"){state="play";$("pauseScreen").classList.add("hidden");last=performance.now();raf=requestAnimationFrame(loop)}}
...
function activateTurbo(){if(state!=="play"||turboCooldown>0||turboEnergy<25)return;turbo=true;turboCooldown=1.6;burst(p.x-20,p.y,12,"#7fe8ff")}
function endTurbo(){turbo=false}
function loseLife(){lives--;combo=1;comboTimer=0;shake=.35;burst(p.x,p.y,20,"#ff7777");if(lives<=0){over();return false}p.x=W*.16;p.y=Math.max(50,Math.min(H-50,H*.5));p.vy=0;p.shield=2;obs=obs.filter(o=>o.x>W*.25);nearMissText="LIFE LOST";nearMissFlash=1;return true}
function awardNearMiss(){combo=Math.min(8,combo+1);comboTimer=3;const bonus=50*combo;score+=bonus;nearMissText="NEAR MISS  +"+bonus;nearMissFlash=1.1;burst(p.x+10,p.y,12,"#ffd34d")}
function update(dt){
 ...
 for(const o of obs){
  ...
  if(topHit||bottomHit){if(p.shield>0){p.shield=0;o.x=-200;burst(p.x,p.y,15,"#6de1ff")}else{if(!loseLife())return;break}}
 }
 obs=obs.filter(o=>o.x>-70);items=items.filter(i=>i.x>-50);
 for(let i=items.length-1;i>=0;i--){let q=items[i];if(Math.hypot(p.x-q.x,p.y-q.y)<26){if(q.shield){p.shield=8;score+=80*combo;burst(q.x,q.y,12,"#6de1ff")}else{coins++;combo=Math.min(8,combo+1);comboTimer=3;score+=60*combo;burst(q.x,q.y,8,"#ffd34d")}items.splice(i,1)}}
 if(p.shield>0)p.shield-=dt;parts.forEach(q=>{q.x+=q.vx*dt;q.y+=q.vy*dt;q.vy+=160*dt;q.life-=dt});parts=parts.filter(q=>q.life>0);hud()
}
